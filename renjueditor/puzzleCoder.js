window.puzzleCoder = (() => {
	 ...
	 * * @randomRotate	true | false		出题时，是否随机翻转
	 * * @mode			{number}			解题模式
	 * *
	 * /
	mode	模式
	...
	*/
	...
	const demoRenjuJSON = `{
		"defaultSettings": {
 		...
 		},
 		{
 			...
 			"level": 1,
 			"title": "一手胜型点",
 			"comment": "找全一手胜型点\一手胜形：指一子落下能够形成成立的基本胜形（活四，四三，四四，三三，抓禁），注意VCF算多手棋"
 		},
 		{
	...
	async function puzzles2RenjuJSON(puzzles, callback = () => {}) {
		...
		for (let i = 0; i < _puzzles.length; i++) {
			...
			Object.keys(puzzle).map(key => {
				if (keys.indexOf(key) == -1) puzzle[key] = undefined;
			})
			puzzle["stones"] = puzzle["stones"] || undefined;
			puzzle["blackStones"] = puzzle["blackStones"] || undefined;
			puzzle["whiteStones"] = puzzle["whiteStones"] || undefined;
			puzzle["title"] = puzzle["title"] || undefined;
			puzzle["comment"] = puzzle["comment"] || undefined;
			puzzle[`//第${("0000"+(i+1)).slice(-(_puzzles.length).toString().length)}题模式`] = MODE_TITLE[puzzle.mode];
			await wait(0);
	...
	/**
	 * 传入一个 JSON 文件，返回一个 Puzzles 对象
	 * @file		file 对象
	 * @callback	回调函数，输出进度
 	*/
	async function loadJSON2Puzzles(file, callback = () => {}) {
		let puzzles;
		const json = await file.text();
		if (json.indexOf("puzzles") + 1)
		...
		puzzles.defaultSettings.title = puzzles.defaultSettings.title || file.name.slice(0, file.name.lastIndexOf("."));
		return puzzles;
	}
