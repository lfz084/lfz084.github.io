window.makeVCF = (function() {
    try {
        ...
        function getVCF(idx) {
            if (idx >= 0 && idx < vcfGames.length) {
                vcfIndex = idx;
                return code2BoardArr(vcfGames[vcfIndex], path)
            }
        }

        function nextVCF() {
            if (vcfIndex < vcfGames.length - 1) {
                return getVCF(++vcfIndex);
            }
        }
        ...
        function resetMakeVCF(boardArr, bStoneCount, wStoneCount) {
            ...
            color = bStoneCount == wStoneCount ? 1 : 2,
                minLen = Math.max((stoneCount >>> 1), 3);
            vcfGames.length = 0;
            vcfIndex = -1;
            putMin();
                ...
                while (true) {
                    ...
                    while (true) {
                        while (state == STATE_SEARCHING && count < 5000 && ps.length < 6 && nextParam(param)) {
                            ...
                            if (!(count & 0xff)) cBoard.unpackArray(param.arr);
                            await wait(0)
                            ps.push(window.engine.findVCF(param)
                                .then(info => {
        ...
        return {
            wait: wait,
            resetMakeVCF: resetMakeVCF,
            stop: stop,
            continueMakeVCF: continueMakeVCF,
            nextVCF: nextVCF,
            preVCF: preVCF,
