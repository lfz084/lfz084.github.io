window.makeVCF = (function() {
    "use strict";
        ...
        const vcfGames = (() => {
            ...
            function hasWinMove(game) {
                for (let i = games.length - 1; i >= 0; i--) {
                    const winMove = games[i].winMove;
                    if (isEq(winMove, game.winMove)) return true
                }
                return false;
            }
            ...
            function getGame(idx) {
                if (idx >= 0 && idx < filterArr.length) {
                    ...
                    return { array: code2BoardArr(game.code, path), winMove: game.winMove, firstColor: game.firstColor }
                }
            }
        ...
        function putStone() {
            ...
            if (color >= 1 && color <= 2) {
                stones[color] == 0 && (color = INVERT_COLOR[color]);
        ...
        function resetMakeVCF(boardArr, bStoneCount, wStoneCount) {
            ...
            color = bStoneCount == wStoneCount ? 1 : 2;
                minLen = Math.max(~~(stoneCount / 2.5), 3);
            vcfGames.reset();
            putMin();
        ...
        function nextGameCode() {
            let gameCode = null;
            while (true) {
                while (true) {
                    let col = putStone(),
                        isBack = (col != 1 && col != 2) || depth == stoneCount;
                    if (isBack) {
                        takeStone();
                        if (depth == stoneCount - 1) {
                            gameCount++;
                            gameCode = searchCode.slice(0, stoneCount);
                            break;
                        }
                        if (depth < 0) {
                            putMax();
                ...
                let dp = gameCode ? backDepth(gameCode) : -1; //如果有(五连，冲四...)局面就跳过所有子局面
                if (dp + 1) while(dp < depth) {takeStone()}
                else break;
            }
            return gameCode;
        }

        function nextParam(param) {
            let gameCode = nextGameCode();
            if (gameCode) {
                param.arr = code2BoardArr(gameCode, path);
                param.color = color;
                param.maxVCF = 1;
                param.maxDepth = 180;
                param.maxNode = 100000;
            }
            return !!gameCode;
        }
        ...
        async function continueMakeVCF() {
            try {
                while (true) {
                    let ps = [],
                        count = 0,
                    ...
                    while (true) {
                        while (state == STATE_SEARCHING && count < 5000 && ps.length < 6 && nextParam(param)) {
                            let code = searchCode.slice(0, stoneCount);
                            if (!(count & 0xfff) && cBoard) cBoard.unpackArray(param.arr);
                            ps.push((async () => {
                                ...
                                if (info.winMoves[0] && info.winMoves[0].length > minLen) {
                                    ...
                                    !info.winMoves[0] && vcfGames.pushGame({code:code, winMove:winMove , firstColor: nParam.color});
                                }
                            })())
                            count++;
                        }
